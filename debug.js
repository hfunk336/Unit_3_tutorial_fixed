function test(){
    ...
    // missing end quote and semi-colon
    ...
    for(var x in list){
        if (list[x] !== "turn" && list[x] !== "through"){
            // var for arrays do not have quotes
            // !=== is not a boolean
    ...
    for(var x = 0; x < filteredList.length; x+=3){
        ...
        // arrays use brackets not parenthesis
        end.push(filteredList[x+2]);
        // List wasn't capital L
    }
    ...
    for(var i =0; i < instructions.length; i++){
        if(instructions[i] === "on"){
            ...
            yS = tempS;
            yE = tempL;
            // case sensitive var, t in tempL is lowercase
            ...
            var xS = start[i][0];
            var yS = start[i][1];
            // first word of a var is lowercase
            ...
            tempS = Math.min(xS, xE);
            // doesn't need a closed parenthesis
            tempL = Math.max(xS, xE);
